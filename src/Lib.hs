...
someFunc = do
  ...
  input <- getLine
  if input == "1"
    then -- One player, set the singlePlayer flag to true
      gameLoop emptyBoard 'x' True
    else
      if input == "2"
        then gameLoop emptyBoard 'x' False
        else
          if input == "exit"
            then return ()
            else do
...
restart = do
  ...
  playAgain <- getLine
  let decision = [toLower x | x <- playAgain]
  if decision == "y"
    then do
      putStrLn $ replicate 20 '\n'
      someFunc
    ...
    then do
      ...
      if isWinner newBoard cpuMove
        then do
          ...
          if catsGame newBoard
            then do
              putStrLn $ boardStr newBoard
              putStrLn "Tie game!"
              restart
            else -- No win, no tie: continue playing, set the playerchar to 'x'
              gameLoop newBoard (nextChar playerChar) singlePlayer
    else -- 2 Player game or players turn for single player game
    do
      putStrLn $ "\n" ++ boardStr board ++ "\n"
        ...
        then return ()
          ...
          if snd move
            then do
              ...
              if snd newBoardTuple
                then do
                  -- The newBoard var contains the updated board
                  let newBoard = fst newBoardTuple
                  -- Check for winner
                  if isWinner newBoard (fst move)
                    then do
                      putStrLn $ boardStr newBoard
                      ...
                      if catsGame newBoard
                        then do
                          ...
                          putStrLn "Tie Game!"
                          restart
                        else -- No win, no tie: continue game
                          gameLoop newBoard (nextChar playerChar) singlePlayer
                else -- The move was parsed properly, but rejected by the board
            ...
            else -- The move was invalid (parser could not determine it)
            do
...
-- | Returns the state of the board after
